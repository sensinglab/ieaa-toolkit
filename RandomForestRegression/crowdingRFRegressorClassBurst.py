...
try:
    ...
    if not df_raw.empty:
        ...
        #Classification
        cols_to_ignore = ['MAC', 'Time', 'Seq']
        df_clean = df_raw.drop_duplicates(subset=[c for c in df_raw.columns if c not in cols_to_ignore])
        ...
        if unique_classes == 0:
            ...
            bursts_per_class = total_bursts / unique_classes
        ...
        X_live = pd.DataFrame([{
            ...
            'Bursts_Per_Class': bursts_per_class
        }])
