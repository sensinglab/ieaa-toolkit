...
try:
    ...
    if not df_raw.empty:
        ...
        if unique_fingerprints == 0:
            ...
            macs_per_fingerprint = 0
        else:
            packets_per_fingerprint = total_packets / unique_fingerprints
            bursts_per_fingerprint = total_bursts / unique_fingerprints
            macs_per_fingerprint = unique_macs / unique_fingerprints

        if unique_macs == 0:
            packets_per_mac = 0
            bursts_per_mac = 0
        else:
            packets_per_mac = total_packets / unique_macs
            bursts_per_mac = total_packets / unique_macs

        if total_bursts == 0:
            packets_per_burst = 0
        else:
            packets_per_burst = total_packets / total_bursts
        ...
        X_live = pd.DataFrame([{
            ...
            'Packets_Per_MAC': packets_per_mac,
            'Packets_Per_Fingerprint': packets_per_fingerprint,
            'Bursts_Per_MAC': bursts_per_mac,
            'Bursts_Per_Fingerprint': bursts_per_fingerprint,
            'MACs_Per_Fingerprint': macs_per_fingerprint
        }])
